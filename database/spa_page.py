...
class SPAComponent(BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if prompt:
            ...
            try:
                log.info(f"Generated component {output}")
    ...
    @jsonapi_attr
    def project_id(self):
        project_id = Path(__file__).parent.parent.name
        return project_id
    
    @jsonapi_attr
    def tsx_path(self):
        #return Path(f"{ULID_ROOT}/{self.project_id}/ui/spa/gen_components/{self.id}/HighLight.tsx")
        return Path(f"{ULID_ROOT}/{self.project_id}/ui/spa/src/components/sections/HighLight.tsx")
...
def add_templates(session):
    ...
    directory_path = "/opt/webgenai/simple-spa/src/components/sections"

    # Iterate over all files in the directory
    for file_path in sorted(glob.glob(f"{directory_path}/Resource*")):
